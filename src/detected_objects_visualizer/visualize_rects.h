...
private:
  ...
  message_filters::Subscriber<autoware_tracker::DetectedObjectArray> *detection_filter_subscriber_;
  ...
  message_filters::sync_policies::ApproximateTime<sensor_msgs::Image,
    autoware_tracker::DetectedObjectArray> SyncPolicyT;
  ...
  SyncedDetectionsCallback(
    ...
    const autoware_tracker::DetectedObjectArray::ConstPtr &in_range_detections);

  bool IsObjectValid(const autoware_tracker::DetectedObject &in_object);

  cv::Mat ObjectsToRects(cv::Mat in_image, const autoware_tracker::DetectedObjectArray::ConstPtr &in_objects);
